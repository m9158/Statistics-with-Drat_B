...
1. 표본 수가 충분한가?
 ...
 6개월의 근무 시간동안, 한번의 평가를 했는지 꾸준한 평가를 했는지에 대한 확인이 필요하다. 따라서 표본수가 충분한가 라는 체크리스트는 정당한 평가였는지 확인하는데 좋은 지표가 될 수 있다고 생각한다.

2. 평가 점수의 평균뿐 아니라 분산(표준편차)는 어떤가?

만약 여러번의 근무 평가가 이루어졌다면, 분산 혹은 평균을 보는 것이 괜찮은 전략일수도있다. 하지만, 분산이 낮은 쪽은 꾸준하게 잘해서 점수가 좋게 나올수도 있고, 분산이 높은 경우, 한 두번 결정적인 역할을 해내서 점수가 좋게 나올 수 도 있기 때문에, 이것만 보고 유무능을 판단하기에는 각각의 상황이 더 중요하다고 생각된다.

3. 데이터가 정규분포를 다른다고 가정할 수 있는가?

대답은 x 이다. 몇번의 근무평가를 시행했는지와, 분산 혹은 평균이 어떠한지 알 수 없는 상황에서는 정규분포를 따른다고 보기에는 섣부른 판단이 된다.

4. 두 사람의 수치 차이에 대해 통계적 검정을 해야 하는가?

B가 A에게 영향을 주고 받는 것에 대한 통계적 검정을 할 필요는 있다고 본다. 만약 A의 수치들과 B의 수치들이 상관관계를 갖는다면, 그것도 그거대로 역시 공정한 판정이라고 하기는 어렵다. 따라서 확인해보면 좋은 정도라고 생각한다. 만약 각 근무 평가 수치들과 점수의 통계적 검정을 진행한다고 한다면, 그것은 타당하다고 볼 수 있다.
```
